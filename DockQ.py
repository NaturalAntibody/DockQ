...
def common_atoms(ref_model, sample_model, atom_for_sup):
...
def collect_common_interface(sample_model, atoms_def_in_both, interface, atom_for_sup):
    common_interface = []
    sample_atoms = []
    chain_res = {}
    for sample_chain in sample_model:
    ...
    return common_interface, sample_atoms, chain_res


def collect_ref_atoms(ref_model, common_interface, atom_for_sup, chain_res, atoms_def_in_both):
    chain_ref = {}
    ref_atoms = []
    common_residues = []
    ...
    return ref_atoms, common_residues, chain_ref


def collect_chain_sample(sample_model, atom_for_sup, common_residues, atoms_def_in_both):
...
def calc_DockQ(model, native, use_CA_only=False, capri_peptide=False):
    ...
    atoms_def_in_both = common_atoms(ref_model, sample_model, atom_for_sup)
    common_interface, sample_atoms, chain_res = collect_common_interface(sample_model, atoms_def_in_both, interface, atom_for_sup)

    ref_atoms, common_residues, chain_ref = collect_ref_atoms(ref_model, common_interface, atom_for_sup, chain_res,
                                                              atoms_def_in_both)

    chain_sample = collect_chain_sample(sample_model, atom_for_sup, common_residues, atoms_def_in_both)
            ...
            for g1 in combos1:
                for g2 in combos2:
                    ...
                    if (test_dict['DockQ'] > best_DockQ):
                        ...
                        best_g1 = g1
                        best_g2 = g2
                        best_info = 'Best score ( ' + str(best_DockQ) + ' ) found for model -> native, chain1:' + ''.join(
                            best_g1) + ' -> ' + ''.join(nat_group1) + ' chain2:' + ''.join(best_g2) + ' -> ' + ''.join(nat_group2)
